...
public class Backtester
{
    ...
    public Backtester(string _path)
    {
        this._path = _path;
    }
    ...
    private async Task<List<BinanceKline>?> GetHistoricalData(string symbol, DateTime start, DateTime end)
    {
        using (var client = new HttpClient())
        {
            ...
            try
            {
                query = $"{Configuration.BinanceApiUrl}?symbol={symbol}&interval={Configuration.Interval}&startTime={startTime}&endTime={endTime}";
    ...
    private BacktestResult SimulateTrade(TradeSignal signal, List<BinanceKline> data)
    {
        ...
        decimal liquidationPrice = Convert.ToDecimal(signal.Position == "LONG"
            ? Helper.CalculateLongLiquidationPrice(signal.Entry, Configuration.InitialBalance, Configuration.Leverage)//* (1 - Configuration.LiquidationPercentage)
            : Helper.CalculateShortLiquidationPrice(signal.Entry, Configuration.InitialBalance, Configuration.Leverage));
        ...
        foreach (var kline in data)
        {
                ...
                (signal.Position == "SHORT" && kline.HighPrice >= liquidationPrice))
            {
                ...
                result.Pnl = -Configuration.InitialBalance;
                return result;
            }
    ...
    private decimal CalculatePnl(TradeSignal signal, decimal exitPrice)
    {
        ...
        decimal positionSize = Configuration.InitialBalance * Configuration.Leverage / signal.Entry;
